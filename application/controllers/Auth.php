...
class Auth extends CI_Controller
{
	...
	public function registration()
	{
		...
		$valid->set_rules(
			'nik',
			'KTP',
			'required|trim|is_unique[user.nik]|min_length[16]|max_length[16]',
			[
				'required'		=> '%s is required!',
		...
		$valid->set_rules(
			'email',
			'Email',
			'required|trim|valid_email|is_unique[user.email]|min_length[8]|max_length[30]',
			[
				'required'		=> '%s is required!',
				'is_unique'		=> '%s <strong>' . $this->input->post('email') .
					'</strong> has been already taken.Try another one!',
				'min_length'	=> '%s min 8 characters!',
				'max_length'	=> '%s max 30 characters!'
			]
		);
		...
		$valid->set_rules(
			'password1',
			'Password',
			'required|trim|min_length[8]|matches[password2]|max_length[15]',
			[
				'required'		=> '%s is required!',
				'min_length'	=> '%s too short!',
